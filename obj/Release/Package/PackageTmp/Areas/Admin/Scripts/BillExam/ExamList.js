...
//新增
function Added() {
    window.location.href = "/Admin/BillExam/Add";
}
//编辑
function Edit(EId, E_IsState) {
    ...
    window.location.href = '/Admin/BillExam/Edit?EId=' + EId;
}
...
function Show(EId, E_Kind) {
    ...
    $.ajax({
        url: '/Admin/BillExam/GetListById?EId=' + EId,
        type: 'POST',
        dataType: 'json',
        async: false,
        success: function (data) {
            if (data.length > 0) {
                $("#Span1").html(data[0]["ExamName"]);
                ...
                var txtE_Type = '考试模式';
                if (parseInt(data[0]["ExamPattern"]) == 1) {
                    txtE_Type = '练习模式';
                }

                $("#Span3").html(txtE_Type);//考试模式
                $("#Span4").html(data[0]["TimeLong"]);//考试时长

                var txtE_IsTimeBonus = "是";
                if (parseInt(data[0]["TimePlus"]) == 2) {
                    txtE_IsTimeBonus = '否';
                }
                $("#Span5").html(txtE_IsTimeBonus);//是否时间加分

                if (E_Kind == "2") {
                    var E_TeamId = data[0]["GradeID"] + "";
                    var teamlist = E_TeamId.substring(1, E_TeamId.length - 1);//去掉前后 ，
                    $.ajax({
                        url: '/Admin/BillExam/GetListTeamByInId?Id=' + teamlist,
                        type: 'POST',
                        dataType: 'json',
                        async: false,
                        success: function (d) {
    ...
    $.ajax({
        ...
        success: function (tb) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                html += '<td><span class="pie" >' + data[i]["ExamName"] + '</span></td>';
                var txtE_Type = '考试模式';
                if (parseInt(data[i]["ExamPattern"]) == 1) {
                    txtE_Type = '练习模式';
                }
                ...
                var txtE_IsState = '';
                if (parseInt(data[i]["Bill_Spare1"]) == 1) {
                    txtE_IsState = '已激活';
                }

                if (parseInt(data[i]["Bill_Spare1"]) == 2) {
                    txtE_IsState = '未激活';
                }
                html += '<td><span class="pie">' + txtE_IsState + '</span><input type="hidden"  id="IsStateId' + data[i]["ID"] + '" value="' + data[i]["Bill_Spare1"] + '"/></td>';


                html += '<td>';
                //查看 中看种类区分E_Kind
                html += '<a href="javascript:void(0);" onclick="Show(' + data[i]["ID"] + ')" class=" btn-info btn-sm"><i class="fa fa-lightbulb-o m-r-xxs"></i>查看 </a>';
                html += '<a style="margin-left: 5px;"  href="javascript:void(0);" onclick="Edit(' + data[i]["ID"] + ',' + data[i]["Bill_Spare1"] + ')" class=" btn-primary btn-sm"><i class="fa fa-pencil m-r-xxs"></i>编辑 </a> ';
                html += '</td>';
