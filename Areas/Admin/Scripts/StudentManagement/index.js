...
function bindTeam() {
    $.ajax({
        ...
        async: false,
        success: function (data) {
            var html = '<option value="0">请选择支行名称</option>';
            for (var i = 0; i < data.length; i++) {
                html += '<option value="' + data[i]["Id"] + '" title="' + data[i]["TeamName"] + '" >' + (data[i]["TeamName"].length <= 10 ? data[i]["TeamName"] : data[i]["TeamName"].substring(0, 10) + "..") + '</option>';
...
function BtnSubim() {
    ...
    if (AddTeamId == "0") {
        layer.msg('请选择支行！', function () { });
        return;
    }
    if (AddStudentNo.length == 0) {
        layer.msg('请输入员工编号！', function () { });
        return;
    }
    if (AddStudentNo.length > 15) {
        layer.msg('员工编号长度不能大于15位！', function () { });
        return;
    }

    if (AddStudentName.length == 0) {
        layer.msg('请输入员工姓名！', function () { });
        return;
    }
    if (getByteLen(AddStudentName)> 20) {
        layer.msg('员工姓名长度不能大于20个汉字！', function () { });
        return;
    }
...
function BatchBtnSubim() {
    ...
    if (BatchS.length == 0) {
        layer.msg('请输入员工开始编号！', function () { });
        return;
    }
    if (BatchE.length == 0) {
        layer.msg('请输入员工结束编号！', function () { });
        return;
    }
    if (BatchS.length > 15) {
        layer.msg('员工开始编号长度不能大于15位！', function () { });
        return;
    }
    if (BatchE.length > 15) {
        layer.msg('员工结束编号长度不能大于15位！', function () { });
        return;
    }
    if (parseInt(BatchS) > parseInt(BatchE)) {
        layer.msg('员工开始编号不能大于结束编号！', function () { });
        return;
    }
    if (parseInt(BatchAddStudentNum) > 100) {
        layer.msg('员工编号区间不能大于100！', function () { });
        return;
    }

    //不自动关闭 time:-1  亮度页面不可点击 shade:0.01
    layer.msg('正在努力生成员工信息，请稍等', {
    ...
    $("#BatchAddform").ajaxSubmit({
        ...
        success: function (data) {
            ...
            if (data == "0") {
                ...
                layer.alert('操作成功,批量新增了' + data + '个员工！');
            }
        }
    });
}
...
function EditBtnSubim() {
    ...
    if (getByteLen(EditStudentName) > 20) {
        layer.msg('员工姓名长度不能大于20个汉字！', function () { });
        return;
    }
