...
function checkform() {
    ...
    if ($("#CollegeName").val() == "") {
        layer.msg('分行名称是必填项！', function () { });
        return false;
    }
    if (getByteLen($("#CollegeName").val()) > 40) {
        layer.msg('分行名称不能大于20个汉字！', function () { });
        return false;
    }
    //if (chkstr == "") {
    ...
    if (College >= 1) {
        layer.msg('您好,不能添加重复的分行名称！', function () { });
        return false;
    }
    return true;
}
...
function EditSave() {
    ...
    $.ajax({
        ...
        data: { Id: $("#EId").val(), CollegeName: CollegeName, Jurisdiction: "1," },
        success: function (data) {
            if (data == "-1") {
                layer.msg('您好,不能添加重复的分行名称！', { icon: 2 });
                return;
            } else if (data > 0) {
...
function Dele() {
    ...
    layer.confirm('您确定要删除所选分行吗？', {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == "-1") {
                    layer.msg('不可执行，请先删除该分行下的支行以及分行管理员用户和员工用户！', { icon: 2 });
                    return;
                } else if (data > 0) {
                    layer.msg('操作成功！', { icon: 1 });
                } else {
