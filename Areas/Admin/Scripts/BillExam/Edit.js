...
function GetListById() {
    $.ajax({
        Type: "post",
        dataType: "json",
        url: '/Admin/BillExam/GetListById?Eid=' + getQueryString("EId"),
        async: false,
        success: function (data) {
            if (data.length > 0) {
               
                $("#selectId").val(data[0]["PaperID"]);
                $("#EamxName").val(data[0]["ExamName"]);
                ...
                //考试时长
                $("#EditE_Whenlong").val(data[0]["TimeLong"]);

                //是否时间加分
                var E_IsTimeBonus = data[0]["TimePlus"];
...
function BindDll() {
    $.ajax({
        url: '/Admin/BillExam/EditGetListTeam',
        type: 'POST',
        dataType: 'json',
        async: false,
        success: function (data) {
