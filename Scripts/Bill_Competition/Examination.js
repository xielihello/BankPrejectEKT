    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = 0; i < jsond.length; i++) { //循环添加div  
                var divObj = $('<div> <input class="bills_input" autocomplete="off" ondragleave="return false;" ondragenter="return false;" ondragover="return false;" onpaste="return false" type="text" id="Bills' + i + '" name="Bills' + i + '" value=\'\'  /></div>'); //定义一个div
                divObj.attr({
                    "id": i,
                    "rel": jsond[i].fwTool,
                }).css({ //改变div样式
