...
function Getinto(Eid, Pid) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "1") {
                window.open('/HB_kaoshi?Eid=' + Eid + '&Pid=' + Pid);
            } else {
...
function ViewGrowthTrajectory(Eid) {
    ...
    $.ajax({
        type: "POST",
        dataType: "json",
        url: '/HB_Competition/GeViewGrowthTrajectory',
