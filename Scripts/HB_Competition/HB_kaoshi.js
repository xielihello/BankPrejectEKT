        ...
        success: function (data) {
            ...
            if (data != null && data.length > 0) {
                ...
                for (var y = 0; y < data.length; y++) {
                    ...
                    var EP_Custom3 = data[y]["EP_Custom3"];//是否标记
                    ...
                    var stylestr = '<img class="biaozhu" id="imgkz' + QBId + '" style="display:none;" src="/img/student/icobiaoji.png"/>';

                    if (parseInt(EP_Custom3) > 0) {
                        stylestr = '<img class="biaozhu" id="imgkz' + QBId + '" style="display:block;" src="/img/student/icobiaoji.png"/>';
                    }

                    html += '<li class="num_sty"  onclick="djth(' + QBId + ',' + y + ')"><span class="num_bj color_no" id="YXthxz' + QBId + '">' + thNum + stylestr + '</span></li>';
                }
...
function setbj(num) {
    $.ajax({
        url: "/HB_kaoshi/setbj",
        data: { "QuestionBId": xTid, "Pid": Pid, "num": num, "Eid": Eid },
        type: 'POST',
        async: false,
        dataType: 'json',
        success: function (data) {

            if (num == 1) {//原本是 标记的 那就取消
                $("#titleDescription").html(dqtitile + '<button class="btn btn-danger btn-sm" type="button" onclick="setbj(0)">取消标记</button>');
                $("#imgkz" + xTid).css('display', 'block');

            }
            else {

                //显示
                $("#titleDescription").html(dqtitile + '<button class="btn btn-success btn-sm" type="button" onclick="setbj(1)">标记此题</button>');
                //隐藏
                $("#imgkz" + xTid).css('display', 'none');
...
function Bindinfo(QBId) {
    ...
    $.ajax({
        ...
        data: { "QuestionBId": QBId, "Pid": Pid, "Eid": Eid },
        type: 'POST',
        async: false,
        dataType: 'json',
        success: function (data) {
            if (data != null && data.length > 0) {
                ...
                var EP_Custom3 = data[0]["EP_Custom3"];//是否为标记
                var stylestr = '<button class="btn btn-danger btn-sm" type="button" onclick="setbj(0)">取消标记</button>';
                if (EP_Custom3 == null || EP_Custom3 == "" || EP_Custom3 == "0") {//没有标记或者取消了标记
                    stylestr = '<button class="btn btn-success btn-sm" type="button" onclick="setbj(1)">标记此题</button>';
                }

                //当前题多少分
                $("#newQBId").html(data[0]["EP_Score"]);
                $("#titleDescription").html(xuhao + "、" + HTMLDecode(data[0]["QB_Description"]) + stylestr);//题目描述
                dqtitile = xuhao + "、" + HTMLDecode(data[0]["QB_Description"]);
                var daanHtml = "";
